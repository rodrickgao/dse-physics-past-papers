...
import { PRIMARY_YEAR, pageHash, readRoute, readSaved, writeSaved } from "./lib/study-state";
...
function Sidebar({ page, navigate, dark, toggleDark, language, mistakeCount }) {
  return <aside className="sidebar">
    ...
    <nav className="nav-list" aria-label="Main navigation"><p className="nav-eyebrow">{language === "eng" ? "STUDY SPACE" : "學習空間"}</p>{NAVIGATION.map(({ id, zh, en, note, icon: Icon }) => <button key={id} aria-current={page === id ? "page" : undefined} className={cn("nav-item", page === id && "active")} onClick={() => navigate(id)}><Icon /><span><strong>{language === "eng" ? en : zh}</strong><small>{note}</small></span>{id === "mistakes" && mistakeCount > 0 ? <em className="nav-count">{mistakeCount}</em> : page === id && <span className="nav-dot" />}</button>)}</nav>
...
function Topbar({ language, setLanguage, submitSearch, page }) {
  ...
  return <header className="topbar">{page === 'papers' ? <div className="workspace-breadcrumb"><Atom/><span>DSE Physics</span><ChevronRight/><strong>{language === 'eng' ? 'Past papers' : '真題庫'}</strong></div> : <form className="global-search" onSubmit={submit}><Search /><input ref={search} value={query} onChange={(event) => setQuery(event.target.value)} placeholder={language === "eng" ? "Search year, question or concept" : "搜尋年份、題號或知識點"} aria-label="Global search" /><kbd><Command /> K</kbd></form>}<div className="topbar-actions"><Button variant="ghost" size="icon" onClick={() => setLanguage(language === "eng" ? "chn" : "eng")} aria-label="Switch language"><Languages /></Button><button className="language-pill" onClick={() => setLanguage(language === "eng" ? "chn" : "eng")}>{language === "eng" ? "ENG" : "中文"}</button></div></header>;
}

function HomeView({ language, navigate, mistakeCount, openRandom }) {
  const isEnglish = language === "eng";
  const sections = [
    { id: "papers", index: "01", title: isEnglish ? "Past papers" : "真題庫", description: isEnglish ? "2015–2025: question, official answer, then worked reasoning. Earlier papers remain in the archive." : "2015–2025：題目、官方解答、詳細思路逐步展開。早期試卷另存資料區。", metric: entriesByLanguage[language].filter(e => Number(e.year.year) >= PRIMARY_YEAR).length, metricLabel: isEnglish ? "questions · 2015–2025" : "道題 · 2015–2025", icon: FileQuestion, tone: "blue" },
...
export function App() {
  ...
  const openRandom = () => { const entries = entriesByLanguage[language].filter(e => Number(e.year.year) >= PRIMARY_YEAR && e.question.answerText !== 'Deleted'); if (entries.length) openPaper(entries[Math.floor(Math.random() * entries.length)].key); };
