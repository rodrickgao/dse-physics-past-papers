...
export function App() {
  ...
  const navigate = useCallback((next) => { setPage(next); if (next !== "papers") setPaperQuery(""); if (next === 'papers') setPaperTarget(lastPaper.current); const hash = pageHash(next, lastPaper.current); if (location.hash !== hash) history.pushState({ page: next }, "", hash); window.scrollTo({ top: 0, behavior: "instant" }); }, []);
